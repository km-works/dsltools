 ...
 *
 * <p>
 * The code generator makes the following assumptions about the intermediate language:<ul>
 *
 * <li>The entire grammar is contained in a single {@link Module
 * module}.</li>
 *
 * <li>All imported types have been registered with {@link
 * AST#importType} and all imported modules have been registered with {@link AST#importModule}.</li>
 *
 * <li>If the grammar references any types that may be {@link
 * Locatable locatable} (as opposed to definitely being or not being locatable), the grammar must have a {@link Properties#LOCATABLE
 * locatable} property with value <code>Boolean.TRUE</code>.</li>
 *
 * <li>If the grammar contains any {@link GenericValue generic
 * values}, the grammar must have a {@link Properties#GENERIC generic} property with value
 ...
 * GenericRecursionValue generic recursion values}, the grammar must have a {@link Properties#RECURSIVE recursive}
 * property with value <code>Boolean.TRUE</code>.</li>
 *
 * <li>Each {@link Production production} must be a {@link
 * FullProduction full production} and must have been annotated with the appropriate {@link MetaData meta-data}.</li>
 *
 * <li>Generally, all {@link Option options}, {@link Repetition
 * repetitions}, and nested {@link OrderedChoice ordered choices} must have been desugared into equivalent productions.
 * However, an ordered choice may appear as the <i>last</i> element of a sequence (that is <i>not</i> part of a
 * predicate), a repetition may appear if the list of repeated expressions need not be memoized, an an option may appear
 * if its value is not bound or depends on a single bound element.</li>
 *
 * <li>The element of a {@link Repetition repetition} must be a {@link
 * Sequence sequence} (with the last element possibly being an ordered choice; see previous assumption). If the
 * repetition is bound, {@link Analyzer#bind(List)} must be able to capture the semantic value of the repeated
 * element.</li>
 *
 * <li>The element of a {@link Option option} must be a {@link
 * Sequence sequence} (with the last element possibly being an ordered choice). If the option is bound,
 * {@link Analyzer#bind(List)} must be able to capture the semantic value of the optional element.
 * </li>
 *
 * <li>The element of a {@link StringMatch string match} must be a {@link NonTerminal nonterminal}.</li>
 *
 * <li>The element of a {@link FollowedBy} or {@link NotFollowedBy} predicate must be a sequence.</li>
 *
 * <li>All elements in a {@link CharSwitch character switch} must either be ordered choices or sequences. Furthermore,
 * character switches may only appear as the last element in a sequence and not within predicates.</li>
 *
 * </ul>
 *
 * @author Robert Grimm
...
public class CodeGenerator extends Visitor {
    ...
    /**
     * The name of the variable referencing the element value for bound repetitions, or <code>null</code> if no such
     * repetition is currently being processed.
     */
    ...
    /**
     * The name of the variable referencing the element value for bound options, or <code>null</code> if no such option
    ...
    /**
     * Extern the specified type. If the specified type is not <code>null</code>, this method returns its external
    ...
    public void visit(Module m) {
        ...
        ///- <CR002/> printer.indent().pln("import xtc.parser.ParserBase;");
        ///   No need to import this class because since <CR002> we extend from
        ///   statically defined (i.e. not generated) abstract class AbstractParser
        ///   which in turn extends xtc.parser.ParserBase. Such ParserBase is
        ///   effectively decoupled from generated code.
...
///- <CR002/>  printer.p("final class ").p(className).pln(" extends ParserBase {").incr().pln();
    ...
    public void visit(FullProduction p) {
        ...
        if (p.hasAttribute(Constants.ATT_PUBLIC)) {
            // Top-level parsing methods are public final.
///- <CR003/> printer.p("public");
            printer.p("public final");  ///+ <CR003/>
        } else {
            // The rest is private.
            printer.p("private");
        }
     ...
     * @param saveIndex The flag for whether to save the index in the base index variable.
     * @param threadError The flag for whether to thread the parse error.
     */
     ...
     * @param isLastChar Flag for whether the just recognized character is the terminal's last character.
     */
    ...
    /**
     * Note that a test has been emitted. This method should be called at the <i>end</i> of the method that emitted the
     * test.
     */
    ...
    /**
     * Emit the code for the next element. If the next element is the last element in the main sequence, the code for
     * returning a semantic value is also emitted.
     *
     * @see #returnValue()
     */
    ...
    private void location() {
        ...
        if (runtime.test("optimizeLocation") && (!createsNodeValue)) {
            return;
        }
     ...
     *
     * @return <code>true</code> if we are processing a not-followed-by predicate.
     */
    protected boolean notFollowedBy() {
    ...
    /**
     * Convert a binding into the corresponding variable name. If the specified binding is for a synthetic variable and
     * the bound element is a null literal, this method returns "null" to inline the null value. Otherwise, it returns
     * the binding's name.
     *
     * @param b The binding.
    ...
    /**
     * Calculate the number of a generic node's children. This method returns the fixed number of children if none of
     * the children has a list value, <code>Integer.MAX_VALUE</code> if any of the children has a non-null list value,
     * and <code>Integer.MIN_VALUE</code> if any of the children has a list value that may be <code>null</code>.
     *
