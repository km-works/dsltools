...
public abstract class ParserBase {
    ...
    /**
     * The start index for lines. Note that this constant mirrors {@link xtc.Constants#FIRST_LINE} to avoid parsers depending on
     * that class.
     */
    public static final int FIRST_LINE = 1;

    /**
     * The start index for columns. Note that this constant mirrors {@link xtc.Constants#FIRST_COLUMN} to avoid parsers
    ...
    // -------------------------------------------------------------------------
    /**
     * Reset this parser to the specified index. This method discards the input and all memoized intermediate results up to and
     * excluding the specified index. The index should be determined by accessing {@link SemanticValue#index} from a previous,
     ...
     * @throws IOException Signals an exceptional condition while accessing the character stream.
     */
     ...
     *
     * @param index The index.
     * @return The corresponding line, without any line terminating characters.
     * @throws IndexOutOfBoundsException Signals an invalid index.
    ...
    /**
     * Set the location for the specified index. This method updates the internal location based on, for example, a line marker
     * recognized by the parser.
     *
     * <p>
     * This method must be called before any nodes are created for positions at or beyond the specified index &mdash; unless the
     * specified file, line, and column are the same as the internal location for the index. The line number may be one less
     * than the start index for lines ({@link #FIRST_LINE}), to account for a line marker being present in the input. The column
     * number is generally be expected to be the start index for columns ({@link
     * #FIRST_COLUMN}), again accounting for a line marker being present in the input.
     *
     * @param index The index.
     ...
     * @throws NullPointerException Signals a null file name.
     * @throws IllegalArgumentException Signals an invalid line or column number.
     * @throws IndexOutOfBoundsException Signals an invalid index.
     * @throws IllegalStateException Signals that the index comes at or before any memoized results.
     */
    ...
    // -------------------------------------------------------------------------
    /**
     * Apply the specified actions on the specified seed. This method applies all {@link xtc.util.Action actions} on the
     * specified list, using the result of the previous action as the argument to the next action. The argument to the first
     * action is the specified seed. If the specified list is empty, this method simply returns the specified seed.
     *
    ...
    /**
     * Apply the specified actions on the specified seed while also setting the results' locations. This method applies all {@link
     * xtc.util.Action actions} on the specified list, using the result of the previous action as the argument to the next
     * action. For the result of each application, it also sets the location. The argument to the first action is the specified
     * seed. If the specified list is empty, this method simply returns the specified seed.
     *
     * @param <T>
     * @param actions The actions to apply.
     * @param seed The initial argument.
     ...
     * @return The result of applying the actions.
     */
    protected final <T extends Locatable> T apply(Pair<Action<T>> actions, T seed, final int index) {
        if (!actions.isEmpty()) {
     ...
     * @throws IOException Signals an I/O error while creating the error message.
     */
    public final String format(ParseError error) throws IOException {
        ...
        if (-1 != error.index) {
            ...
            buf.append(NEWLINE);
            for (int i = FIRST_COLUMN; i < c.column; i++) {
                buf.append(' ');
            }
            buf.append('^');
            buf.append(NEWLINE);
     ...
     * @throws IOException Signals an I/O error while creating the exception's detail message.
     */
     ...
     * SemanticValue}, this method returns the actual value; if it is a {@link ParseError}, it signals a parse exception with
     * the corresponding message. The specified result must have been created by this parser.
     *
     * @param r The result.
     * @return The corresponding value.
     * @throws ParseException Signals that the result represents a parse error.
     * @throws IOException Signals an I/O error while creating the parse error's detail message.
     */
    ...
    // -------------------------------------------------------------------------
    /**
     * Cast the specified object. This method is used to avoid spurious compiler warnings for parsers 
     * utilizing generic types.
     *
     * @param <T> The target type of the cast operation.
     * @param o The object.
    ...
    /**
     * Cast the list starting at the specified pair. This method is used to avoid spurious compiler warnings 
     * for parsers utilizing generic types.
     *
     * @param <T> The target item type of the cast operation.
     * @param p The list.
