 ...
 * @since 1.0
 */
public final class RRPredicate extends SingletonNode {
    
    protected static final int EXT = 3*RAD/2;

    private final boolean negating;

    protected RRPredicate(String predicateType) {
        super();
        negating = predicateType.equals(PredicateType.MustNotMatch.name());
    }
    ...
    public void accept(SVGRenderingVisitor v, RenderingParams params) {
        v.renderPredicate(this, params);
    }

    @Override
    public int getWidth() {
        return EXT + getChild().getWidth();
    }
